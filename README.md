...
lock-on-screensaver setting. When run as root in GNOME, it writes these values to the
local system dconf database and locks them, so they apply to existing and future GNOME
users and cannot be overridden per account. On XFCE it disables idle screensaver
activation, the screensaver and locker master switches, lock-on-screensaver
activation, lock-on-sleep across the screensaver, power-manager, and session channels,
and power-manager DPMS for both AC and battery use. It also disables the active X11
...
Run this task as root from a GNOME graphical session to enforce the policy for every
GNOME user. Otherwise, run it as the intended desktop user from a terminal within the
graphical session. It intentionally weakens a physical-access security control: the
desktop will remain visible while idle. GNOME manual locking remains available; XFCE's
master locker is disabled, so its manual lock command is unavailable too. The task does
not alter automatic suspend/hibernate, lid actions, brightness dimming, low-battery
policy, display-manager login screens, or third-party lockers such as `xscreensaver`
and `light-locker`.
